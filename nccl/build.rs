...
fn main() {
    ...
    if let Some(maca) = find_maca_root() {
        ...
        if let Some(nccl_root) = find_nccl_root() {
            nccl.define();
            include_cuda();
            bind_nccl(cuda_root, nccl_root)
        }
    }
}

fn bind_nccl(cuda: impl AsRef<Path>, nccl: Option<PathBuf>) {
    let mut includes = vec![format!("-I{}/include", cuda.as_ref().display())];
