...
impl KernelFn<'_> {
    ...
    pub fn max_threads_per_block(&self) -> usize {
        self.get_attribute(MC_FUNC_ATTRIBUTE_MAX_THREADS_PER_BLOCK) as _
    }

    #[inline]
    pub fn static_smem(&self) -> MemSize {
        self.get_attribute(MC_FUNC_ATTRIBUTE_SHARED_SIZE_BYTES)
            .into()
    }

    #[inline]
    pub fn max_dyn_smem(&self) -> MemSize {
        self.get_attribute(MC_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES)
            .into()
    }

    #[inline]
    pub fn local_mem(&self) -> MemSize {
        self.get_attribute(MC_FUNC_ATTRIBUTE_LOCAL_SIZE_BYTES)
            .into()
    }

    #[inline]
    pub fn num_regs(&self) -> MemSize {
        self.get_attribute(MC_FUNC_ATTRIBUTE_NUM_REGS).into()
    }

    #[inline]
    pub fn ptx_version(&self) -> Version {
        let version = self.get_attribute(MC_FUNC_ATTRIBUTE_PTX_VERSION);
        Version {
            major: version / 10,
            minor: version % 10,
        }
    ...
    #[inline]
    fn get_attribute(&self, attr: mcFunction_attribute) -> c_int {
        let mut value = 0;
        driver!(mcFuncGetAttribute(&mut value, attr, self.0));
        value
    }
}
