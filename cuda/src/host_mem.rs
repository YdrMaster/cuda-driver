...
fn test_managed() {
    ...
    dev.context().apply(|ctx| {
        ...
        let size = pagable.len();
        let (dev, ptr) = ctx.malloc_managed::<u8>(size);
        unsafe {
            std::ptr::copy_nonoverlapping(pagable.as_ptr(), ptr as *mut u8, size);
        }
        crate::memcpy_d2h(pagable2, &dev);
...
fn test_vir_mem() {
    ...
    dev.context().apply(|ctx| {
        ...
        let size = pagable.len();
        let (mut dev, ptr, padded_size, allochandle) = ctx.malloc_vir::<u8>(size);
        crate::memcpy_h2d(&mut dev, &pagable);
        crate::memcpy_d2h(pagable2, &dev);
        ...
        //释放
        driver!(cuMemUnmap(ptr, padded_size));
        driver!(cuMemRelease(allochandle));
        driver!(cuMemAddressFree(ptr, padded_size));
    });
}
