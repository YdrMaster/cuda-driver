...
use crate::bindings::{mcHostFn_t, mcHostNodeParams};
...
impl Graph {
    ...
    pub fn add_host_node<'a>(
        ...
        let mc_host_node_params = mcHostNodeParams {
            fn_: host_fn,
            userData: user_data,
        };
        let mut node = null_mut();
        driver!(mcGraphAddHostNode(
            &mut node,
            self.as_raw(),
            deps.as_ptr(),
            deps.len(),
...
#[cfg(test)]
mod test {
    use crate::{AsRaw, Ptx, bindings::mcHostNodeParams, graph::Graph, params};
    ...
    fn test_host_graph_node() {
        ...
        crate::Device::new(0).context().apply(|ctx| {
            ...
            let cuda_host_node_params = mcHostNodeParams {
                fn_: Some(host_fn),
